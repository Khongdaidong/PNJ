...
import { Info, RefreshCw, Newspaper } from "lucide-react";
    ...
    const NewsPanel = () => {
        const fallbackNews = [
            ...
            },
            {
                ...
                url: "https://www.pnj.com.vn/quan-he-co-dong/",
                date: "11/2025",
        ...
        const newsUpdatedAt = news.length > 0 ? news[0].date : "N/A";
        ...
        const impactsById = React.useMemo(() => {
            ...
            resolvedNews.forEach((item) => {
                map[item.id] = assessKpiImpact(item);
            });
            return map;
        }, [resolvedNews]);
        ...
        const impactSummary = React.useMemo(() => {
            const summary = {
                stores: { up: 0, down: 0, neutral: 0 },
                sssg: { up: 0, down: 0, neutral: 0 },
                margin: { up: 0, down: 0, neutral: 0 }
            };
            resolvedNews.forEach((item) => {
                const impact = impactsById[item.id] || {
                    stores: neutralImpact("missing"),
                    sssg: neutralImpact("missing"),
                    margin: neutralImpact("missing")
                };
                summary.stores[impact.stores.direction] += 1;
                summary.sssg[impact.sssg.direction] += 1;
                summary.margin[impact.margin.direction] += 1;
            });
            return summary;
        }, [resolvedNews, impactsById]);

        const renderImpactBadge = (label: string, impact: ImpactResult) => {
            const variant = impact.direction === "up" ? "default" : impact.direction === "down" ? "destructive" : "secondary";
            const directionLabel = impact.direction === "up" ? "Up" : impact.direction === "down" ? "Down" : "Neutral";
            const title = `${label}: ${directionLabel} (${impact.confidence}) - ${impact.reason}`;
            return (
                <Badge variant={variant} className="text-[10px] h-5 font-normal" title={title}>
        ...
        return (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <Card className="rounded-2xl shadow-sm h-full">
                        <CardHeader className="pb-3 border-b">
                            <div className="flex items-center gap-2">
                                ...
                                <CardTitle className="text-base">Tin tức & Sự kiện nổi bật (Cập nhật {resolvedUpdatedAt})</CardTitle>
                            </div>
                        </CardHeader>
                        <CardContent className="space-y-0 p-0">
                            {resolvedNews.map((item) => {
                                ...
                                return (
                                    <div key={item.id} className="p-4 hover:bg-muted/30 transition-colors border-b last:border-0">
                                        <div className="flex flex-col md:flex-row gap-3 justify-between md:items-start">
                                            <div className="space-y-1.5 flex-1">
                                                <div className="flex items-start gap-2">
                                                    <div className="font-semibold text-sm text-blue-900 dark:text-blue-300 flex-1">
                                                        {item.url ? (
                                                            <a
                                                                href={item.url}
                                                                target="_blank"
                                                ...
                                                <div className="flex items-center gap-2 pt-1 flex-wrap">
                                                    <Badge variant="outline" className="text-[10px] h-5 font-normal">{getTagLabel(item.tag)}</Badge>
                                                    <span className="text-[11px] text-muted-foreground">{item.source}</span>
                                                    {renderImpactBadge("Stores", impact.stores)}
                                                    {renderImpactBadge("SSSG", impact.sssg)}
                                                    {renderImpactBadge("Margin", impact.margin)}
                                                </div>
                                            </div>
                                            <div className="flex-shrink-0">
                                                <Badge variant="secondary" className="text-[11px] whitespace-nowrap">{item.date}</Badge>
                                            </div>
                                        </div>
                                    </div>
                                );
                ...
                <div className="space-y-6">
                    <Card className="rounded-2xl shadow-sm bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950/20 dark:to-teal-950/20 border-emerald-200 dark:border-emerald-800">
                        <CardHeader className="pb-2">
                            <div className="flex items-center gap-2">
                                <span className="text-lg">KPI</span>
                                <CardTitle className="text-sm text-emerald-800 dark:text-emerald-200">KPI Impact Summary</CardTitle>
                            </div>
                        </CardHeader>
                        <CardContent className="space-y-3">
                            <div className="text-[10px] text-emerald-700 dark:text-emerald-300">
                                Heuristic impact based on tags/keywords. Update news text to refine.
                            </div>
                            <div className="space-y-2 text-xs">
                                <div className="flex items-center justify-between">
                                    <span>Stores</span>
                                    <span className="font-medium text-emerald-800 dark:text-emerald-200">
                                        {impactSummary.stores.up} up / {impactSummary.stores.down} down / {impactSummary.stores.neutral} neutral
                                    </span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span>SSSG</span>
                                    <span className="font-medium text-emerald-800 dark:text-emerald-200">
                                        {impactSummary.sssg.up} up / {impactSummary.sssg.down} down / {impactSummary.sssg.neutral} neutral
                                    </span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span>Margin</span>
                                    <span className="font-medium text-emerald-800 dark:text-emerald-200">
                                        {impactSummary.margin.up} up / {impactSummary.margin.down} down / {impactSummary.margin.neutral} neutral
                                    </span>
                                </div>
                            </div>
                        </CardContent>
                    ...
                    <Card className="rounded-2xl shadow-sm bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/20">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm text-blue-800 dark:text-blue-200">Điểm nhấn đầu tư 2026</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-3">
                            <ul className="space-y-2 text-xs text-blue-900 dark:text-blue-300">
                                <li className="flex gap-2">
                                    <span className="font-bold">•</span>
                                    <span>Sự phục hồi của sức mua tầng lớp trung lưu sau giai đoạn khó khăn 2024-2025.</span>
                                </li>
                                <li className="flex gap-2">
                                    <span className="font-bold">•</span>
                                    <span>Biên lợi nhuận mở rộng nhờ chiến lược cao cấp hoá và tối ưu chi phí vận hành.</span>
                                </li>
                                <li className="flex gap-2">
                                    <span className="font-bold">•</span>
                                    <span>Định giá P/E hiện tại (~14-15x) thấp hơn trung bình lịch sử (17-18x), mở ra cơ hội tích luỹ.</span>
                                </li>
                            </ul>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="rounded-2xl shadow-sm">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm">Lịch sự kiện sắp tới</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-3 text-xs">
                            <div className="flex justify-between items-center border-b pb-2">
                                <span>ĐHCĐ Thường niên 2026</span>
                                <Badge variant="outline">04/2026 (Est)</Badge>
                            </div>
                            <div className="flex justify-between items-center border-b pb-2">
                                <span>KQKD Quý 4/2025</span>
                                <Badge variant="outline">30/01/2026</Badge>
                            </div>
                            <div className="flex justify-between items-center">
                                <span>Chi trả cổ tức đợt 1/2025</span>
